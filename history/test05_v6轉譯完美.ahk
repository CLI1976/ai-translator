...
; 全局變數
global CurrentProvider := "Claude"  ; 預設使用 Claude
...
Translate(targetLanguage, prompt) {
    ...
    ; 創建翻譯的請求內容
    jsonContent := CreateTranslationJson(provider, model, A_Clipboard, prompt)
...
ShowResponse(response, provider, title := "") {
    ; 創建 GUI
    if (title != "") {
        responseGui := Gui("", title)
    } else {
        responseGui := Gui("", provider " 回應")
    }
    ...
    ; 添加翻譯結果
    responseGui.AddText("xm y+10 w200", "處理結果:")
    translatedBox := responseGui.AddEdit("xm w600 h200 ReadOnly")
